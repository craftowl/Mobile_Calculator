...
export const getDecimalPlaces = async (): Promise<number> => {
  try {
    ...
    return savedDecimalPlaces ? parseInt(savedDecimalPlaces, 10) : DEFAULT_DECIMAL_PLACES;
  } catch (error) {
...
export const getHistoryLimit = async (): Promise<number> => {
  try {
    ...
    return savedHistoryLimit ? parseInt(savedHistoryLimit, 10) : DEFAULT_HISTORY_LIMIT;
  } catch (error) {
